---
title: "Shatabdi Express Guide: Timings, Routes & Booking Rules"
description: "Shatabdi Express guide 2026: timings, route schedules, the Tatkal booking window, fares, and which meals come free with your seat. Plan before you book."
date: "2026-06-30"
updated: "2026-07-11"
tags:
  - train booking
  - irctc
  - shatabdi express
  - travel guide
---

## TL;DR
Shatabdi Express is a premium network of daytime air-conditioned chair car trains operated by Indian Railways, connecting major cities. Tickets follow standard IRCTC rules with ARP of 60 days. Tatkal booking opens at 10:00 AM, and catering is optional.

---

## What is the Shatabdi Express and Its Key Features?

**Shatabdi Express trains are a series of premium, fast-daytime passenger trains operated by Indian Railways. Designed for quick intercity travel, they connect major metropolitan cities with key commercial and tourist hubs. These fully air-conditioned trains feature modern chair cars, onboard catering, high-priority schedules, and comfortable seating layouts.**

Shatabdi trains are among the fastest and most reliable options for daytime train travel in India. Unlike long-distance sleeper trains, Shatabdis operate entirely during the day, returning to their originating station on the same day. 

### Modern Passenger Amenities
*   **Seating Classes:** Shatabdis feature two main classes: AC Chair Car (CC) and Executive Chair Car (EC). Executive Class offers wider seats, extra legroom, and superior catering menus.
*   **Priority on Tracks:** Shatabdis are given high priority on the Indian Railways network, second only to Rajdhani and Vande Bharat trains, resulting in fewer delays.
*   **Onboard Hospitality:** Every passenger receives complimentary newspapers, bottled water, and hand sanitizers upon boarding, alongside premium catering services.

---

## What are the Booking Rules and Tatkal Timings for Shatabdi Express?

**Booking rules for Shatabdi Express follow the standard 60-day advance reservation period on IRCTC. Shatabdi Express Tatkal timings open one day before departure at 10:00 AM since these are air-conditioned (AC) trains. No general concessions are applicable on Shatabdi trains except for eligible Divyangjan passenger categories.**

Planning a Shatabdi journey requires understanding these key booking windows:

*   **Advance Reservation Period (ARP):** You can book seats up to **60 days** before the journey date starting at 8:00 AM daily.
*   **Shatabdi Express Tatkal Timings:** Tatkal bookings open **1 day** before the train's scheduled departure from its originating station. Since all Shatabdi coaches are AC (Chair Car and Executive Chair Car), booking opens strictly at **10:00 AM**. Seats under this quota sell out within minutes, so keeping passenger lists ready is essential.
*   **Concession Restrictions:** To keep fares aligned with operational costs, Indian Railways does not offer senior citizen discounts or other concessions on Shatabdi trains. Only passenger-level Divyangjan (disability) concessions are supported under specified quotas.

---

## What are the Main Routes and Schedule of Popular Shatabdi Express Trains?

**Shatabdi Express trains run on high-demand intercity corridors across India, operating on daily or six-days-a-week schedules. Key routes include the New Delhi to Bhopal Shatabdi, New Delhi to Amritsar Shatabdi, and New Delhi to Kalka Shatabdi. They cover distances between 300 to 700 kilometers during daytime hours.**

Shatabdi trains connect commercial centers and major tourist hubs. The table below outlines key Shatabdi Express routes, their train numbers, and general schedules:

### List of major Shatabdi Express trains, routes and train numbers

Here are the main Shatabdi Express trains across India. Tap a train number to see its full station-by-station schedule and live chart preparation times on LastBerth.

| Route | Train numbers | Schedule & chart times |
| :--- | :--- | :--- |
| Bhopal (Rani Kamlapati) ↔ New Delhi | 12001 / 12002 | [12001 schedule](/chart-times/12001-ndls-shatabdi-chart-times) |
| Lucknow ↔ New Delhi (Swarna) | 12003 / 12004 | [12004 schedule](/chart-times/12004-shatabdi-express-chart-times) |
| Chennai ↔ Mysuru | 12007 / 12008 | [12007 schedule](/chart-times/12007-mys-shatabdi-chart-times) |
| Mumbai Central ↔ Ahmedabad | 12009 / 12010 | [12009 schedule](/chart-times/12009-adi-shatabdi-exp-chart-times) |
| New Delhi ↔ Ajmer | 12015 / 12016 | [12015 schedule](/chart-times/12015-ajmer-shatabdi-chart-times) |
| Howrah ↔ Ranchi | 12019 / 12020 | [12019 schedule](/chart-times/12019-shatabdi-exp-chart-times) |
| Pune ↔ Secunderabad | 12025 / 12026 | [12025 schedule](/chart-times/12025-shatabdi-exp-chart-times) |
| Chennai ↔ KSR Bengaluru | 12027 / 12028 | [12027 schedule](/chart-times/12027-mas-sbc-shatabdi-chart-times) |
| New Delhi ↔ Amritsar (Swarna) | 12029 / 12030 | [12029 schedule](/chart-times/12029-swarn-shatabdi-chart-times) |
| New Delhi ↔ Chandigarh | 12045 / 12046 | [12045 schedule](/chart-times/12045-cdg-shatabdi-chart-times) |
| Guwahati ↔ Dibrugarh | 12085 / 12086 | [12085 schedule](/chart-times/12085-shatabdi-exp-chart-times) |
| Chennai ↔ Coimbatore | 12243 / 12244 | [12243 schedule](/chart-times/12243-mas-cbe-shatabdi-chart-times) |

India also runs several **Jan Shatabdi Express** trains, a more affordable Shatabdi variant with a mix of AC chair car and second-sitting coaches. You can look up any train's timetable and chart timing on the [chart preparation times](/chart-times) page.

*Timings and running days change with the seasonal timetable, so confirm the current schedule before you travel.*

---

## Are Onboard Catering and Food Options Included in Shatabdi Express Fares?

**Yes, onboard catering is included in Shatabdi Express ticket fares, featuring breakfast, lunch, high tea, or dinner depending on journey timings. However, passengers can opt out of meals during booking to reduce ticket costs. Opting in onboard incurs an additional service fee of ₹50 per meal.**

Food is a major highlight of the Shatabdi travel experience. Here is how the catering policy works:

1.  **Catering Opt-Out option:** During ticket booking on IRCTC, you can choose not to receive meals. This will lower your ticket cost by removing the catering fee.
2.  **Onboard Food Requests:** If you opt out of catering but later request meals onboard from the pantry car staff, you must pay the regular meal cost plus an additional **₹50 service charge** per meal.
3.  **Menu Selection:** Passengers can choose between vegetarian and non-vegetarian menus during booking. The meals are prepared under hygienic conditions, featuring continental and regional Indian cuisines.

---

## How to Book Confirmed Shatabdi Express Seats When Waitlisted?

**If Shatabdi tickets are waitlisted, monitor their queue status using LastBerth. E-tickets remaining on the waiting list after chart preparation are auto-cancelled and refunded. You can use LastBerth’s smart features to split journey segments, book current availability tickets, or locate vacant seats on running trains.**

Because of their immense popularity, Shatabdi trains often show waitlists. 

### Understanding waitlist mechanics
The WL full form is Waiting List. When you book a waitlisted ticket, your ticket status progresses through a defined queue system:

$$\text{WL (Waiting List)} \longrightarrow \text{RAC (Reservation Against Cancellation)} \longrightarrow \text{Confirmed}$$

However, since Shatabdi Express trains consist only of seating classes (CC and EC) and do not have sleeper berths, there is **no RAC category** on these trains. Tickets move directly from the waiting list to a fully confirmed seat. 

Waitlist queues like `WL/1` or `WL/10` track your position in line. If your online e-ticket does not get confirmed and remains waitlisted after the final reservation chart is prepared (10 hours before departure), IRCTC automatically cancels it and issues a refund. You cannot travel on a waitlisted e-ticket.

### How LastBerth helps you secure a seat
If your direct route shows a waiting list, do not lose hope. You can use LastBerth’s premium features to find alternative travel arrangements:

*   **[Finding Smart Seats](/)**: This tool finds seat availability across a journey by breaking the seat bookings across multiple coaches or segments where a single direct seat is unavailable on the same train.
*   **[PNR Status Search & Direct Booking](/)**: Check your PNR status, view waitlist confirmation probabilities, and search for alternative train combinations immediately if confirmation looks unlikely.
*   **[Seat Status Coach Journey Lookup](/seat-status)**: Look up specific seats to see exactly from which station to which station they are booked. This lets you identify empty seats on running trains and request the Travelling Ticket Examiner (TTE) to allocate them to you.
*   **Current Availability Booking:** If you are booking at the last minute, check for a `"current available ticket"`. A current available ticket is a **fully confirmed seat** with coach and seat numbers. It opens 10 hours before departure (right after chart preparation) and closes 30 minutes before train departure.

---

## Shatabdi Express Booking Questions (FAQ)

### What is the full form of WL in railway booking?
**WL full form is Waiting List**. It indicates that you do not have a confirmed seat and are placed in a queue.

### When does Tatkal booking open for Shatabdi Express?
Because Shatabdi Express is a fully air-conditioned train consisting of CC and EC classes, Tatkal bookings open at **10:00 AM** daily, exactly one day before the train departs from its originating station.

### What is a current available ticket in Shatabdi Express?
A `"current available ticket"` is a confirmed seat that becomes available for booking online or at counters after the reservation chart is prepared. It opens 10 hours before departure and closes 30 minutes before departure.

### Is food free on the Shatabdi Express?
Food is not free; catering charges are included in the ticket fare by default. However, passengers can opt out of food during booking to reduce their overall ticket cost.

### Can we travel on a waitlisted ticket on Shatabdi?
No. Waitlisted e-tickets are automatically cancelled and refunded by IRCTC after chart preparation. Traveling on a waitlisted e-ticket is illegal and will result in a penalty from the TTE. Only physical waitlisted tickets booked at station counters allow you to board the train.

### How does the waiting list queue progress?
The progression is **WL (Waiting List) → RAC (Reservation Against Cancellation) → Confirmed**. However, on Shatabdi trains, since there are no sleeper berths, there is no RAC category, and tickets move directly from WL to Confirmed status.

### Which train is number 12001?
Train 12001 is the New Delhi to Rani Kamlapati (Bhopal) Shatabdi Express, one of the fastest daytime trains in India. Its return service is 12002. You can see the [12001 route and chart times here](/chart-times/12001-ndls-shatabdi-chart-times).

### What is the New Delhi to Amritsar Shatabdi train number?
The New Delhi to Amritsar Swarna Shatabdi runs as **12029** (New Delhi to Amritsar) and **12030** (Amritsar to New Delhi), stopping at stations like Ambala Cantt, Ludhiana and Jalandhar. See the [12029 schedule](/chart-times/12029-swarn-shatabdi-chart-times).

### Which is the New Delhi to Chandigarh Shatabdi?
The New Delhi to Chandigarh Shatabdi runs as **12045** (New Delhi to Chandigarh) and **12046** (Chandigarh to New Delhi). Check the [12045 schedule and chart times](/chart-times/12045-cdg-shatabdi-chart-times).

### How many Shatabdi Express trains are there in India?
There are over 20 Shatabdi Express routes connecting major cities, plus several Jan Shatabdi Express services. See the table above for the main routes and their train numbers, or search any train on our [chart preparation times](/chart-times) page.

### Which is the fastest Shatabdi Express?
The New Delhi to Bhopal (Rani Kamlapati) Shatabdi, train 12001/12002, is consistently one of the fastest, averaging over 90 km/h across its run and touching much higher speeds on the Agra stretch.
