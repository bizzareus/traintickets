---
title: "IRCTC Master List: Save 20 Passengers for Faster Tatkal"
description: "How to use IRCTC Master List to book Tatkal tickets in 30 seconds. Learn 20-passenger profile limits, Aadhaar pre-verification & speed hacks."
date: "2026-07-06"
updated: "2026-08-13"
tags:
  - train booking
  - irctc
  - master list
  - tatkal booking
  - add passenger
---

> **🔔 Never Miss Final Charting:** Waiting for waitlisted seats to confirm? Track your train's exact station charting schedule and set up a free preparation alert on [Chart Times](/chart-times), or check live vacant berths on [Chart Vacancy](/chart-vacancy). Looking for confirmed split seats? Use [Smart Seats](/).

Every second counts in the Tatkal window. At 10:00 AM sharp for AC classes and 11:00 AM for sleeper, thousands of people are all fighting for the same handful of seats. The people who lose are almost never the ones with a slow internet connection. They are the ones still typing "R-A-J-E-S-H, age 34, male, lower berth" into the passenger form while the confirmed seats vanish.

There is a built-in IRCTC feature that removes that typing entirely: the **Master List**. Set it up once, and on booking day you just tick names off a list. Yet most travellers have never opened it.

Here is exactly how the Master List and Travel List work, what to save, and how to use them to shave 30–60 seconds off every booking.

---

## What is the IRCTC Master List?

**The IRCTC Master List is a pre-saved profile feature that stores up to 20 passenger profiles, including names, ages, genders, berth preferences, and Aadhaar verification details. Pre-saving passenger details eliminates manual typing on booking day, allowing you to select saved names with a single click during high-speed Tatkal openings.**

Think of it as your phone's contacts, but for train bookings. You enter each person once, and IRCTC remembers them forever. This matters most during Tatkal and the general opening rush, where the passenger-entry screen is the single biggest time sink between you and a confirmed berth.

It lives under **My Account → My Profile → Master List** (called "Add/Modify Master List" on the website, and "Master List" under the profile menu in the IRCTC Rail Connect app).

---

## What is the Difference Between the Master List and the Travel List?

**The Master List is your master roster of up to 20 individual passenger profiles stored in your IRCTC profile. In contrast, a Travel List is a pre-grouped custom subset (like "Family" or "Colleagues") created from your Master List to add multiple frequent co-travellers simultaneously with a single tap during booking.**

Here is the simplest way to remember it:

- **Master List** = the pool of everyone you might ever book for (up to 20 individuals).
- **Travel List** = pre-made bundles of those people you travel with together often.

If you almost always book for the same four family members, put them in a Travel List. Then on booking day, choosing that one list drops all four passengers into the form at once. For a solo traveller or someone with an ever-changing group, the Master List alone is usually enough.

---

## How to Create and Verify an IRCTC Master List?

**To create a Master List, log into your IRCTC account, navigate to My Account, select My Profile, and click Add/Modify Master List. Enter each passenger's name, age, gender, berth preference, and Aadhaar number, then submit. Verified status is confirmed once the green tick appears next to the passenger's profile.**

Step by step on the website:

1. Log in to the IRCTC portal.
2. Hover over **My Account** and choose **My Profile**, then **Master List**.
3. Click **Add Passenger**.
4. Enter the passenger's **name (exactly as on their ID)**, age, gender, berth preference, food preference, nationality, and — where required — ID card type and number.
5. Click **Submit / Save**. The passenger now appears in your list.

In the **Rail Connect app**, the path is **Profile → Master List → Add Passenger**, with the same fields.

> **Do this on a lazy Sunday, not on Tatkal morning.** The entire advantage of the Master List disappears if you are still creating it at 9:59 AM. Set it up in advance so booking day is pure selection, not data entry.

---

## What Passenger Details Should You Save in the Master List?

**You should save full names exactly as printed on official ID cards, age, gender, berth preference (lower, middle, upper, side), meal choice (veg/non-veg), and Aadhaar number for identity verification. Accurate pre-saved details ensure seamless Tatkal processing and prevent name mismatch penalties during onboard TTE checking.**

The fields you can pre-save include:

| Field | Why it matters |
| --- | --- |
| **Name** | Must match a valid photo ID exactly — a mismatch can mean a penalty during checking |
| **Age & Gender** | Drives senior-citizen fares and lower-berth auto-allocation |
| **Berth preference** | Lower / Middle / Upper / Side Lower, etc. — set your usual choice |
| **Food preference** | Veg / Non-veg / no food — only used on trains with catering like Rajdhani, Shatabdi, Vande Bharat |
| **Nationality** | Indian or foreign national |
| **ID card type & number** | Needed for senior-citizen, Aadhaar 24-ticket limit, and quota bookings |

Getting the name spelling right here pays off twice: it saves typing, and it prevents the [name mismatch problems](/blog/irctc-name-correction-spelling-age-gender-rules) that get flagged during ticket checking.

---

## How Does the Master List Speed Up Tatkal Booking?

**The Master List speeds up Tatkal booking by replacing manual typing of passenger names, ages, and preferences with instant checkbox selections. During high-concurrency Tatkal windows (10:00 AM AC, 11:00 AM Sleeper), this saves 30 to 60 crucial seconds, allowing you to reach payment gateways before confirmed quotas exhaust.**

The Tatkal booking sequence, compared:

- **Without a Master List:** train → class → *type every passenger's name, age, gender, preference by hand* → captcha → payment. The typing step is slow and error-prone under pressure.
- **With a Master List:** train → class → **tick saved passengers** → captcha → payment. The slowest manual step is gone.

That saved time compounds with the other Tatkal fundamentals — logging in early, having your payment method ready (UPI or a saved card), and knowing the exact opening time for your class. Pair the Master List with the tactics in our [Ultimate Tatkal Booking Guide](/blog/ultimate-tatkal-booking-guide-speed-hacks) for the full speed setup.

Once you have booked, track the result instantly with [Smart Seats](/) and our fast PNR Status search, and use the [Coach Journey Lookup](/seat-status) to see the live berth position along your route.

---

## Can You Edit or Delete Passengers from the Master List?

**Yes, you can edit or delete saved profiles anytime under My Profile in your IRCTC account settings. Since the Master List is capped at 20 passengers, deleting inactive contacts keeps your list organized and frees up slots to add new travellers prior to Tatkal bookings.**

Keep the list current: update a child's age each year (it affects fares and berth eligibility), fix any spelling you later notice, and remove one-off co-passengers after the trip. A clean, accurate list is the one that actually saves you time when it matters.

---

## Does the Master List Sync Across IRCTC App and Website?

**Yes, the IRCTC Master List is linked directly to your central user ID, automatically syncing across the IRCTC Rail Connect app and official web portal. Creating profiles on one platform makes them immediately available on the other, ensuring flexible booking across devices during high-traffic Tatkal rushes.**

This account-level sync is genuinely useful on Tatkal morning: if the website is overloaded, you can switch to the app (or vice versa) and your saved passengers are already there. For more on choosing between the two under pressure, see our comparison of the [IRCTC app versus website for Tatkal](/blog/irctc-app-vs-website-tatkal-booking).

---

## Common Booking Questions (FAQ)

### How many passengers can I add to the IRCTC Master List?

You can save up to 20 passengers in your IRCTC Master List. This is separate from the per-ticket limit (a maximum of 6 passengers per booking, or 4 during Tatkal). If you hit 20, delete passengers you no longer book for to make room for new ones.

### Is Aadhaar pre-verification mandatory for Master List passengers?

Aadhaar pre-verification is required if you want to unlock the expanded 24-ticket monthly booking limit or participate in priority peak-hour Tatkal bookings. For standard booking limits, unverified Master List entries are allowed.

### Is the Master List the same as booking history?

No. Booking history is a record of tickets you have already purchased. The Master List is a pre-saved directory of passenger details you set up in advance, specifically so you can fill the booking form instantly on future bookings without retyping anyone's information.

### Do I still have to select a berth preference if it is saved in the Master List?

The berth preference you save is applied automatically when you add that passenger, but you can override it on any individual booking. Saved preferences are a default to save time, not a lock — change them per journey whenever you need to.

### Can I use the Master List for Tatkal tickets?

Yes, and it is where the Master List helps most. Tatkal opens at 10:00 AM for AC classes and 11:00 AM for sleeper, and seats sell out in minutes. Selecting pre-saved passengers instead of typing them is one of the biggest time savings available in the Tatkal window.

### Will the Master List auto-fill ID proof for senior citizens?

If you save the ID card type and number for a passenger, those details are stored and can be applied when you book under an ID-linked concession or quota. You still choose the relevant quota and concession option at booking time for that journey.

### Does the Master List cost anything?

No. Creating and using the Master List and Travel List is a free, built-in feature of every IRCTC account. There is no charge to save passengers or to select them during booking.

### What happens if the passenger name on ticket doesn't match ID?

If the name on the ticket differs significantly from the photo ID presented to the TTE, the passenger is treated as traveling without a valid ticket under Section 138, incurring a penalty plus fare. Always verify Master List names against official IDs.

---

## Related Guides

- [The Ultimate Tatkal Booking Guide: Speed Hacks for Confirmed Tickets](/blog/ultimate-tatkal-booking-guide-speed-hacks)
- [IRCTC App vs Website: Which Is Faster for Tatkal Booking?](/blog/irctc-app-vs-website-tatkal-booking)
- [IRCTC Name Correction: Rules for Spelling, Age, and Gender Changes](/blog/irctc-name-correction-spelling-age-gender-rules)
- [IRCTC Ticket Booking Limits: How Many Tickets Can You Book?](/blog/irctc-ticket-booking-limits-aadhaar-verification)
