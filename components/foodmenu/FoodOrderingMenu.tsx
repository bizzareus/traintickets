  ...
  return (
    <div className="pb-16">
      ...
      <div className="space-y-8">
        {services.map((s) => {
          const dishes = s.items.filter((it) => !NON_FOOD.test(it.item) && !NON_FOOD.test(it.description));
          return (
            <section key={s.service} id={`cat-${slug(s.service)}`} className="scroll-mt-28">
      ...
      <p className="mt-8 rounded-lg bg-slate-50 p-3 text-xs text-slate-500">
        Preview UI · dish images are placeholders. Menu served on a cyclic basis;
        items and prices may change. Source: official IRCTC menu.
      </p>
    </div>
  );
}
