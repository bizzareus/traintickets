...
export class WhatsappController {
  ...
  @Post()
  async handleIncoming(@Req() req: Request, @Body() body: Record<string, unknown>) {
    ...
    if (secret) {
        ...
        if (signature) {
           ...
           if (!crypto.timingSafeEqual(Buffer.from(signature, 'utf8'), Buffer.from(`sha256=${expected}`, 'utf8'))) {
    ...
    try {
      this.logger.debug("Received WA Webhook:", JSON.stringify(body).substring(0, 500));
      ...
      messageText = data.text || data.message || data.body || data.message?.conversation || "";
      ...
      if (messageText && sender) {
         this.whatsappService.handleIncomingMessage(sender, messageText, groupId).catch(e => {
