...
export class WhatsappService {
  ...
  async handleIncomingMessage(
    ...
    // 1. NLP Parse
    const parsed = await this.openai.parseTicketRequest(
      messageText,
      new Date(),
    );
    ...
    if (!parsed.origin || !parsed.destination || !parsed.date) {
      await this.sendReply(
        sender,
        groupId,
        `Please specify the origin, destination, and date (e.g. "Delhi to Mumbai tomorrow")`,
      );
      return;
    }
    ...
    try {
      ...
      if (!fromCode || !toCode) {
        await this.sendReply(
          sender,
          groupId,
          `Sorry, couldn't resolve the station codes for ${parsed.origin} to ${parsed.destination}.`,
        );
        return;
      }

      // Perform Train Search
      const trainsData = (await this.booking.searchTrains(
        fromCode,
        toCode,
        dateDdMmYyyy,
      )) as Record<string, any>;
      ...
      if (!trains.length) {
        await this.sendReply(
          sender,
          groupId,
          `No trains found from ${fromCode} to ${toCode} on ${dateDdMmYyyy}`,
        );
        return;
      }
      ...
      await this.sendReply(
        sender,
        groupId,
        `Sorry, ran into an error while finding tickets for you.`,
      );
    }
  }
  ...
  private async sendReply(
    ...
    if (!wasenderUrl || !wasenderKey) {
      this.logger.warn(
        `WASENDER config missing! Would have sent to ${groupId || toId}: \n${message}`,
      );
      return;
    }
