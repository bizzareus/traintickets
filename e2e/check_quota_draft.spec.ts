...
test('scan rewritten emergency quota draft', async ({ page }) => {
  ...
  const draftPath = '/Users/kartikarora/.gemini/antigravity/brain/277b85b5-2fee-4241-a7b4-5c7b18723e29/scratch/quota_rewritten.txt';
  ...
  const startTemplateIdx = lines.findIndex(l => l.trim() === 'To,');
  const endTemplateIdx = lines.findIndex(l => l.trim() === '[Contact Number]');
  ...
  if (startTemplateIdx !== -1 && endTemplateIdx !== -1 && startTemplateIdx < endTemplateIdx) {
    ...
    filteredLines = [
      ...lines.slice(0, startTemplateIdx),
      ...lines.slice(endTemplateIdx + 1)
    ];
  } else {
    console.log('Template indicators not found, scanning entire draft.');
    filteredLines = lines;
  }
  ...
  fs.writeFileSync(path.join(scratchDir, 'validation_text_half_1.txt'), firstHalf);
  fs.writeFileSync(path.join(scratchDir, 'validation_text_half_2.txt'), secondHalf);
  ...
  for (let i = 0; i < halves.length; i++) {
    ...
    await page.goto('https://quillbot.com/ai-content-detector', { waitUntil: 'networkidle' });
    ...
    const screenshotName = `validation_half_${i + 1}.png`;
  ...
  fs.writeFileSync(
    path.join(scratchDir, 'validation_results.json'),
