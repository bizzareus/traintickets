...
const fieldClassName =
  "mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-800 shadow-sm outline-none transition focus:border-teal-600 focus:ring-4 focus:ring-teal-100";
...
export default function TatkalPlannerClient() {
  ...
  return (
    <div className="space-y-5">
      {windowResult && <section className="rounded-2xl border border-teal-100 bg-teal-50/60 p-5 shadow-sm sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><div className="rounded-xl bg-teal-100 p-2.5 text-teal-700"><BellRing className="h-5 w-5" /></div><div><h3 className="font-bold text-slate-900">Want a reminder?</h3><p className="mt-1 text-sm text-slate-600">Get a free reminder 15 minutes before opening.</p></div></div>{!showAlertForm && !subscribeSuccess && <button type="button" onClick={() => setShowAlertForm(true)} className="rounded-xl bg-teal-700 px-4 py-3 text-sm font-bold text-white shadow-sm hover:bg-teal-800 focus:outline-none focus:ring-4 focus:ring-teal-200">Set a free reminder</button>}</div>{subscribeSuccess ? <div className="mt-4 flex items-center gap-2 rounded-xl bg-emerald-50 p-4 text-sm font-semibold text-emerald-800"><CheckCircle2 className="h-5 w-5" /> Reminder set successfully.</div> : showAlertForm && <form onSubmit={handleSubscribeAlert} className="mt-5 grid gap-3 sm:grid-cols-[1fr_1fr_auto]"><label className="sr-only" htmlFor="alert-email">Email address</label><div className="relative"><input id="alert-email" type="email" value={emailInput} onChange={(event) => setEmailInput(event.target.value)} placeholder="Email address" className={`${fieldClassName} mt-0 pr-10`} /><Mail className="pointer-events-none absolute right-3 top-4 h-4 w-4 text-slate-400" /></div><label className="sr-only" htmlFor="alert-mobile">WhatsApp number</label><div className="relative"><input id="alert-mobile" type="tel" value={mobileInput} onChange={(event) => setMobileInput(event.target.value)} placeholder="WhatsApp number" className={`${fieldClassName} mt-0 pr-10`} /><Phone className="pointer-events-none absolute right-3 top-4 h-4 w-4 text-slate-400" /></div><button type="submit" disabled={subscribing} className="inline-flex items-center justify-center gap-2 rounded-xl bg-teal-700 px-5 py-3 text-sm font-bold text-white hover:bg-teal-800 disabled:opacity-60">{subscribing ? <><Loader2 className="h-4 w-4 animate-spin" /> Saving</> : "Save reminder"}</button>{subscribeError && <p className="text-xs font-medium text-rose-600 sm:col-span-3">{subscribeError}</p>}</form>}</section>}
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
        ...
        <div className="grid gap-5 md:grid-cols-3">
          ...
          <div><span className="text-sm font-bold text-slate-900">Your train class</span><div className="mt-2 grid grid-cols-2 gap-2" role="group" aria-label="Train class">{(["AC", "NON_AC"] as const).map((category) => <button key={category} type="button" onClick={() => setClassCategory(category)} className={`rounded-xl border px-3 py-3 text-sm font-bold transition ${classCategory === category ? "border-teal-700 bg-teal-700 text-white" : "border-slate-200 bg-white text-slate-700 hover:border-teal-300"}`}>{category === "AC" ? "AC" : "Sleeper / 2S"}<span className={`mt-1 block text-xs font-medium ${classCategory === category ? "text-teal-100" : "text-slate-500"}`}>{category === "AC" ? "Opens at 10 AM" : "Opens at 11 AM"}</span></button>)}</div></div>
        ...
        {windowResult && <div className="mt-8 overflow-hidden rounded-2xl bg-slate-900 text-white"><div className="p-5 sm:p-7"><p className="text-sm font-semibold text-teal-300">Your Tatkal booking opens</p><div className="mt-2 grid gap-5 sm:grid-cols-[1fr_auto] sm:items-end"><div><p className="text-3xl font-black tracking-tight sm:text-4xl">{formatReadableDateIST(windowResult.tatkalBookingDateStr)}</p><p className="mt-1 text-lg font-bold text-white">at {windowResult.tatkalOpeningTimeFormatted}</p></div><div className="rounded-xl bg-white/10 px-4 py-3 sm:min-w-44 sm:text-right"><p className="text-xs font-semibold uppercase tracking-wide text-slate-300">{countdown.live || countdown.past ? "Status" : "Time left"}</p><p className={`mt-1 font-mono text-xl font-black ${countdown.live ? "text-emerald-300" : "text-amber-300"}`}>{countdown.text}</p></div></div><div className="mt-5 grid gap-3 border-t border-white/10 pt-4 text-xs text-slate-300 sm:grid-cols-2"><p><Clock3 className="mr-1 inline h-3.5 w-3.5 text-amber-300" /> Be ready by <strong className="text-white">{windowResult.recommendedLoginTime}</strong></p><p><Info className="mr-1 inline h-3.5 w-3.5 text-teal-300" /> Train starts on <strong className="text-white">{formatReadableDateIST(windowResult.originDepartureDateStr)}</strong></p></div></div><div className={`px-5 py-3 text-sm font-bold ${countdown.live ? "bg-emerald-500 text-white" : countdown.past ? "bg-slate-800 text-slate-300" : "bg-teal-700 text-white"}`}>{countdown.live ? "Booking is open now — book on IRCTC." : countdown.past ? "This booking window has closed." : "Keep your passenger details ready before opening time."}</div></div>}
      </section>
