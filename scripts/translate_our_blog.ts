...
async function run() {
  ...
  const filename = 'vande-bharat-last-minute-booking-15-minutes-rule.md';
  ...
  for (const lang of LANGUAGES) {
    ...
    console.log(`Translating into ${lang.name} (${lang.code})...`);
    ...
    try {
      ...
      console.error(`   ❌ Failed to translate to ${lang.code}:`, err instanceof Error ? err.message : String(err));
    }

    await new Promise((resolve) => setTimeout(resolve, 2000));
  }

  console.log('🎉 Retranslation of Vande Bharat 15-minute blog complete.');
}
